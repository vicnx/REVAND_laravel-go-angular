...
type ArticleUserModel struct {
	...
	UserModelID    uint
	Items  []Item  `gorm:"ForeignKey:AuthorID"`
...
type TagModel struct {
	...
	Items []Item `gorm:"many2many:article_tags;"`
}
...
func (article Item) favoritesCount() uint {
	db := common.GetDB()
	var count uint
...
func (article Item) isFavoriteBy(user ArticleUserModel) bool {
	db := common.GetDB()
...
func (article Item) favoriteBy(user ArticleUserModel) error {
	db := common.GetDB()
	var favorite FavoriteModel
...
func (article Item) unFavoriteBy(user ArticleUserModel) error {
	db := common.GetDB()
...
func FindOneArticle(condition interface{}) (Item, error) {
	db := common.GetDB()
	var model Item
	tx := db.Begin()
...
func (self *Item) getComments() error {
	db := common.GetDB()
	tx := db.Begin()
...
func FindManyArticle(tag, author, limit, offset, favorited string) ([]Item, int, error) {
	db := common.GetDB()
	var models []Item
	var count int
	...
	if tag != "" {
		...
		if tagModel.ID != 0 {
			tx.Model(&tagModel).Offset(offset_int).Limit(limit_int).Related(&models, "Items")
			count = tx.Model(&tagModel).Association("Items").Count()
		}
	} else if author != "" {
		...
		if articleUserModel.ID != 0 {
			count = tx.Model(&articleUserModel).Association("Items").Count()
			tx.Model(&articleUserModel).Offset(offset_int).Limit(limit_int).Related(&models, "Items")
		}
	} else if favorited != "" {
		var userModel users.UserModel
...
func (self *ArticleUserModel) GetArticleFeed(limit, offset string) ([]Item, int, error) {
	db := common.GetDB()
	var models []Item
	var count int
...
func (model *Item) setTags(tags []string) error {
	db := common.GetDB()
...
func (model *Item) Update(data interface{}) error {
	db := common.GetDB()
	err := db.Model(model).Update(data).Error
	return err
}

func DeleteItem(condition interface{}) error {
	db := common.GetDB()
	err := db.Where(condition).Delete(Item{}).Error
	return err
}
